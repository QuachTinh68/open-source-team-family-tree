				...
				return {
					...
					"render": function(data, type, row, meta) {
						if (columnInfo.field != null) {
							...
							var action = '';
							if(arrURL.urlDetail){
								action += `<a href="#" onclick="openInfoModal('${row[columnInfo.name]}')" class="btn btn-icon edit-color" th:title="Thông tin chi tiểt ${row[columnInfo.name]}">
											<em class="icon ni ni-eye"></em></a>`;
							}
							if(arrURL.urlEdit){
								action += `<a href="${arrURL.urlEdit + row[columnInfo.name]}" class="link-edit btn btn-icon edit-color" th:title="Sửa thông tin ${row[columnInfo.name]}">
